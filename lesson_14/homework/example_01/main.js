...
// где элементы это обьекты, описывающие неких людей. 
// У каждого обьекта обязательно есть свойство "name". 
// Наших массив players как пример подходит. Второй аргумент - это строка. 
// Входящая строка должна состоять только из одной буквы.

// Функция должна вернуть массив из обьектов, отфильтрованных так,чтобы были только обьекты
// где имя начинается на букву, которая переданна вторым аргументом.
let players = [
...
const namesFilterByChar = (arr, letter) => {

    const whatEver = player => {
        return player.name.toLowerCase().startsWith(letter);

    }

    const xxx = arr.filter(whatEver);
return xxx;
}

const result = namesFilterByChar(players, 'k');
console.log(result);



//     const newArr=  arr.filter(player => player.name.toLowerCase().startsWith(letter));
// return newArr;
